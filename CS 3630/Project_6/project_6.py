# -*- coding: utf-8 -*-
"""Project 6 Student.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1mbvWWxverO-DInPR1lrCtX7pfLIjdu7F
...
def transform_cloud(bTa, clouda):
  ...
  use numpy concatinate and numpy ones
    Args:
    ...
    """
    trans = bTa.matrix()
    uno = np.ones((1,clouda.shape[1]))
    homo = np.concatenate((clouda, uno), axis=0)
    ndarray = np.dot(trans,homo)
    W = ndarray[3]
    ndarray = ndarray/W[None,:]
    return ndarray[:-1]
...
def assign_closest_pairs(clouda, cloudb):
    ...
    """    
    nbrs = NearestNeighbors(n_neighbors=1, algorithm='ball_tree').fit(cloudb.T)
    distances, indices = nbrs.kneighbors(clouda.T)
    ndarray = cloudb[:,indices[0]]
    return ndarray
...
def populate_factor_graph(graph, initial_estimates, initial_pose, clouds):
    ...
    for i in range(len(clouds) - 1):
        ...
        #print(clouds[i].shape)
        T,icp_series = icp(clouds[i],clouds[i+1], prev_T)
    ...
    for i in range(0, len(clouds) - 2, 2):
        # TODO: Run ICP between clouds (hint: use inital_tranform argument)
        bTa,icp_series = icp(clouds[i],clouds[i+2], prev_T)
        # TODO: Set T to its inverse: use `gtsam.Pose3.inverse()`
        T = gtsam.Pose3.inverse(T)
        # TODO: Add a `gtsam.BetweenFactorPose3()` to the graph
        graph.add(gtsam.BetweenFactorPose3())
