...
//
    ...
    for (int i = 0; i < res.length; i++) {
      res[i] = q[i]+(Kp*et[i]);
      if (res[i] > Math.PI) {
        res[i] = res[i] - 2 * Math.PI;
      } else if (res[i] < -Math.PI) {
        res[i] = res[i] + 2 * Math.PI;
      }
    }    
...
//
    ...
    double[] et = {xd - tool.x(), yd - tool.y(), theta_d-tool.theta()};//Need to check theatas
    ...
    //also use as reference https://dellaert.github.io/20S-3630/notes/3-manipulators.pdf
    for (int i = 0; i < res.length; i++) {
      res[i] = q[i] + Kp * (et[0] * invJac[i][0] + et[1] * invJac[i][1] + et[2] * invJac[i][2]);
      if (res[i] > Math.PI) {
        res[i] = res[i] - 2 * Math.PI;
      } else if (res[i] < -Math.PI) {
        res[i] = res[i] + 2 * Math.PI;
