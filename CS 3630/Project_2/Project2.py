...
Original file is located at
    https://colab.research.google.com/drive/1M4T03xk39-ek44tvcfj1pjC7L8MDcj7z

# Project 2: Probabilistic Inference

Due Date: Monday, February 3, 2020 @ 11:59 P.M.

Student Name: Ashley Eggart
...
def transition_model(T, S, A):
  ...
  returns:
      P(T|S,A) (float).
  """
  #Determines the correct State if A is taken from S
  if A == "Left" and S[1] != 0:
    cT = (S[0],(S[1]-1))
  elif A == "Left" and S[1] == 0:
    cT = S
  elif A == "Right" and S[1] != 9:
    cT = (S[0],(S[1]+1))
  elif A == "Right" and S[1] == 9:
    cT = S
  elif A == "Up" and S[0] != 0:
    ...
    cT = S
  elif A == "Down" and S[0] != 9:
    cT = ((S[0]+1),S[1])
  elif A == "Down" and S[0] == 9:
    cT = S

  prob = 0
  possList = possible_states(S)
  #Returns the relevant probality
  if T == cT:
    prob = prob + 0.85
  if T in possList:
    prob = prob + 0.05 * possList.count(T)
  if T == cT and T in possList:
    prob = prob - 0.05

  return round(prob,2)

#Define all of the possible states for a given state
def possible_states(S):
  """Given a value of state determine all of the possible states.
  arguments:
      S (int,int): a tuple (i,j), representing the position in the grid.
  returns:
      possList (list).
  ...
  #Check left
  if S[1] != 0:
    possList[0] = (S[0],(S[1]-1))
  #Check right
  if S[1] != 9:
    possList[1] = (S[0],(S[1]+1))
  #Check up
  if S[0] != 0:
    possList[2] = ((S[0]-1),S[1])
  #Check down
  if S[0] != 9:
    possList[3] = ((S[0]+1),S[1])
...
# Create the function cdf- maybe right?
def calculate_cdf(pmf, ordered_outcomes):
  ...
  """
  pmfList = []
  cdfdict = {}#maybe a thing I need?
  for x in ordered_outcomes:
    temp = pmf(x)
    pmfList.append(temp)
    
  pmfArray = np.asarray(pmfList)#maybe?
  cdfArray = np.cumsum(pmfArray)#maybe?-makes a cdf array?
  i = 0
  for x in ordered_outcomes:
    cdfdict[x] = cdfArray[i]
    i += 1
  def cdf(outcome):
    return cdfdict[outcome]#maybe will work because nested???
...
def sample_from_pmf(pmf, ordered_outcomes):
  ...
  """
  cdfFunc = calculate_cdf(pmf, ordered_outcomes)
...
def sample_from_sensor_model(state):
  ...
  for x in OBSERVATIONS:
    tempProb = sensor_model(x,state)
    pmfList.append(tempProb)

  pmfArray = np.asarray(pmfList)
  cdfArray = np.cumsum(pmfArray)#Should be working correctly
...
def sample_from_transition_model(state, action):
  ...
  """
  pass
  u = generate_random_float()#needed for something
